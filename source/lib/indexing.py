...
def IndexSearchMultiple(data, idx, verbose=False, texts=None, print_errors=False):
    nl = len(data)
    nbex = data[0].shape[0]
    cosine = np.zeros((nbex, nbex)).astype(float)
    distances = []
    indexes = []
    ...
    for i1 in range(nl):
        for i2 in range(nl):
            if i1 != i2:
                ...
                indexes.append(I.tolist() )
                
    for i in range(data[0].shape[0]):
        for j in range(data[1].shape[0]):
            cosine[i,j] = np.dot(data[0][i], data[1][j]) / (np.sqrt(np.dot(data[0][i],data[0][i])) * np.sqrt(np.dot(data[1][j],data[1][j]))) 
    
    distances = [[ i[0] for i in j] for j in distances]
    indexes = [[ i[0] for i in j] for j in indexes]
