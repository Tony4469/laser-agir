...
if [ ! -d dev ] ; then
  ...
  tar -xf dev.tgz "dev/newstest2012.??"
  /bin/rm dev.tgz
fi
echo ${LASER}//source/similarity_search.py \
    --bpe-codes ${bpe_codes} --encoder ${encoder} \
    --base-dir . \
    --data dev/newstest2012 --output ${edir}/newstest2012 \
    --lang cs de en es fr --verbose

#python ../../source/similarity_search.py --bpe-codes ../../models/93langs.fcodes --encoder ../../models/bilstm.93langs.2018-12-26.pt --base-dir . --data dev/souf --output embed/souf --lang en fr --verbose
